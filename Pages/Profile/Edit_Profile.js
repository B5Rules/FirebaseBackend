import { StatusBar, StyleSheet, Text, SafeAreaView, TextInput, Picker, ImageBackground, TouchableOpacity, Image, View, ScrollView } from 'react-native';
import React, { useState } from 'react';

import { useValidation } from 'react-native-form-validator';

let getFirstName = () => { return 'Iulian'; }
let getLastName = () => { return 'Manole'; }
...
export default function EditProfile({ navigation }) {
    ...
    const [selectedValue, setSelectedValue] = useState( getCountry() );
    const [fName, setFName] = useState( getFirstName() );
    const [lName, setLName] = useState( getLastName() );
    const [username, setUsername] = useState( getUsername() );
    const [email, setEmail] = useState( getEmail() );
    const [number, setNumber] = useState( getPhoneNumber() );
  
    const { validate, isFieldInError} =
      useValidation({
        state: { fName, lName, username, email, number}
    });
    ...
    const onPressButton = () => {
      ...
      if( !isFieldInError('fName') && fName != "" && 
        ...
        !isFieldInError('number') && number != "") afiseaza(lName, fName, username, email, number, password, confirmPassword);
      };
          ...
          {/* <ImageBackground source={require("../../Images/streets02.png")} resizeMode="cover" style={styles.image}> */}
            ...
            <View style={{flexDirection:'row', alignItems:'center', justifyContent: 'flex-start', marginLeft: '10%', width: '100%'}}>
              ...
              <View style={{flexDirection: 'column', maxWidth: '60%'}}>
                  <Text style={{fontSize: 22, color: 'grey', alignSelf: 'flex-start'}}> Personal profile </Text>
                  <TextInput 
                  editable={false} 
                  textAlign={'center'} 
            ...
            <TouchableOpacity style={styles.button} onPress={() => {onPressButton; navigation.navigate("ViewProfile")}} >
              <Text style={styles.textButton}> Done </Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.buttonCancel} onPress={() => navigation.navigate("ViewProfile")}>
