...
const getStationsGraph = httpsCallable(fireFunc, "getStationsGraph");
...
export const routeCalculator = async (startingPoint, destinationPoint) => {
    ...
    stationsId[nr+1] = [startingPoint.latitude, startingPoint.longitude, ''];
    stationsId[nr+2] = [destinationPoint.latitude, destinationPoint.longitude, ''];
    const distancesBetweenStations = (await getStationsGraph())?.data?.result;
    ...
    for(let index=0;index<=nr+3;index++)
    {
        ...
        if(i!=nr+1){
            ...
            for(let index=1;index<=nr+2;index++){
                ...
                if(index==nr+1 || index==nr+2 || nearestVertex==nr+1 || nearestVertex==nr+2)
                    ...
                    edgeDistance = distancesBetweenStations[stationsId[nearestVertex][2]]?.distances[stationsId[index][2]] || 99999999;
                }
